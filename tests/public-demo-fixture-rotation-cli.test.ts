...
describe("public demo fixture rotation command", () => {
  ...
  it.each([
    ...
    expect(() =>
      resolvePublicDemoFixtureRotationRequest(["--from-version", "2"], environment)
    ).toThrow(message);
  });
  ...
  it("refuses implicit, stale, or malformed version transitions", () => {
    ...
    expect(() =>
      resolvePublicDemoFixtureRotationRequest(["--from-version", "1"], validEnvironment)
